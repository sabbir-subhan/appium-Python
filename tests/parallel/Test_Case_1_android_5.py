...
class TC1android5(unittest.TestCase):
    def setUp(self):

        logging.info("WebDriver request initiated. Waiting for response, this may take a while.")
        desired_capabilities = DesiredCapabilities.desired_capabilities_for_android_5
        # choose desired capabilities from desired_capabilities.py
        self.driver = webdriver.Remote("http://localhost:4736/wd/hub", desired_capabilities)
    ...
    def test_logging_into_OCA_app_android5(self):
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(TC1android5)
