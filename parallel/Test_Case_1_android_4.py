...
class TC1android4(unittest.TestCase):
    def setUp(self):

        logging.info("WebDriver request initiated. Waiting for response, this may take a while.")
        desired_capabilities = DesiredCapabilities.desired_capabilities_for_android_4
        # choose desired capabilities from desired_capabilities.py
        self.driver = webdriver.Remote("http://localhost:4734/wd/hub", desired_capabilities)
        # port number for Appium node
    ...
    def test_logging_into_OCA_app_android4(self):
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(TC1android4)
