...
CKEDITOR.dialog.add( 'image2', function( editor ) {
	...
	function createPreLoader() {
		...
		return function( src, callback, scope ) {
			addListener( 'load', function() {
				...
				fileprogress.innerHTML = lang.complete;
			...
			var imageURL = src.replace('file://', '').replace('x-wmapp0:', '');
			scope.apisrc = imageURL.replace(Configuration.getAPIDomain(), '');
	        if (device.platform == "Mobile Interface"){
	        	image.setAttribute('src', imageURL);
			} else {
		        new Gopher({url: imageURL, backgroundMode:true}, function(downloadURL){
					if (device.platform === 'Win32NT'){
						downloadURL = 'x-wmapp0:' + downloadURL;
					}
		        	...
		        	fileprogress.innerHTML = lang.loading + kb + 'kb';
		        });
		    }
		};
	}
	...
	function onChangeSrc(scope) {
		...
		if ( value !== widget.data.src ) {
			...
			preLoader( value, function( image ) {
				var dimensions = getNatural( image );
				
				scope.src = image.getAttribute('src');

				scope.getDialog().enableButton('ok');
				...
				widthField.setValue( editor.config.image2_prefillDimensions === false ? 0 : dimensions.width );

				// Fill height field with the height of the new image.
				heightField.setValue( editor.config.image2_prefillDimensions === false ? 0 : dimensions.height );

				// Cache the new width.
				preLoadedWidth = dimensions.width;

				// Cache the new height.
				preLoadedHeight = dimensions.height;
	...
	var hasFileBrowser = !!( config.filebrowserImageBrowseUrl || config.filebrowserBrowseUrl ),
		srcBoxChildren = [
			...
			}*/
			{
				type: 'file',
				id: 'upload',
				label: editor.lang.image.btnUploadSelect,
				style: 'height:20px',
				size: 35,
				onChange: function(event){
					var fi = this.getInputElement();
					if (fi.$.files.length){
						var filename = event.data.value.split('\\').pop().split('/').pop();
						if (!widget.data.alt){
							widget.data.alt = filename;
						}
						var data = new FormData();
						data.append('file', fi.$.files[0]);

						$.ajax({
							url: Configuration.getAPIUrl() + "/mediaupload",
							type: 'POST',
							success: this.uploadCallback.bind(this),
							error: $.proxy(this.uploadFail, this),
							cache: false,
							data: data,
							contentType: false, 
							processData: false
						...
						fileprogress.innerHTML = lang.uploading;
				...
				uploadCallback: function(result, status, xhr){
					var url = xhr.getResponseHeader('Location');
					if (url){
						this.src = Configuration.getAPIDomain() + url;
						onChangeSrc(this);
					}
				},
				commit: function( ){
					
					widget.setData('src', this.src);
					
					widget.setData('alt', widget.data.alt);
					widget.element.$.setAttribute('data-apisrc',this.apisrc);
					
					editor.commands.autogrow.exec();
				}
			}
		];
