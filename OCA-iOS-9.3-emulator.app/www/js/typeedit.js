...
TypeEdit.prototype.handleVisibility = function(field, row){
    ...
    for (var id in field.VisibilityFields){
        $lv.on('fieldChange' + id, $.proxy(this.handleVisibilityChange, this, field, row, $lv));
    }
    this.content.on('rendered', $.proxy(this.handleVisibilityChange, this, field, row, $lv, true));
    row.hide();
    field.defaultApplied = false;
};

TypeEdit.prototype.handleVisibilityChange = function(field, row, $lv, initialRender) {
	var dataObj = {};
	for (var id in field.VisibilityFields){
		var input = $lv.find('.field' + id).data('input');
		if (input) dataObj[id] = input.val();
	}

	if (!this.checkVisibility(field, dataObj)) {
		row.showIf(false);
		return;
	}
	...
	if (this.ID && this.typeData.ApplyDefaultWhenVisible && initialRender !== true && !field.defaultApplied && field.DefaultValue){
		...
		if (typeof data == 'string') {
			data = data.replace(/\$\{((?:\\\\|\\\}|[^\}])+)\}/g,''); //don't show expressions in the field value
		}
...
TypeEdit.prototype.getContext = function(){
	...
	if (this.parentURL) {
		context.ParentURL = Configuration.getAPIPath() + this.parentURL;
	}
