...
var ViewUtil = {
	...
	populateLV: function(lv, data, type, icon, selectorMode, nextURL, extraParams){
		...
		var nextData = false;
		if (data.length > 200) {
			nextData = data.splice(200, data.length - 200); // remove the last 200 items and put them in nextData.
		}
		...
		if (nextURL || nextData){
			this.paginate(lv, type, icon, selectorMode, nextURL, nextData);
		}
	 ...
	 */
	paginate: function($lv, type, icon, selectorMode, nextURL, nextData) {
		var win = $(window);
		...
		document.body.onscroll = function() {
			...
			if ((win.height() + win.scrollTop()) > ($lv.height() + $lv.position().top) * .66){
				...
				$lv.after(newLV);
				if (nextData) {
					ViewUtil.populateLV(newLV, nextData, type, icon, selectorMode, nextURL);
				} else {
