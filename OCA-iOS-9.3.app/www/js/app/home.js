...
var homeController = $.extend({}, app, {
	...
	getDashboard: function(dontCache) {
		...
		var qs = [];
		if (pe) {
			qs.push('current_eme=' + encodeURI(pe.url));
		}
		var qstr = '?' + qs.join('&');
		new Gopher('/me/dashboard' + qstr, $.proxy(this.gotDashboard, this), $.proxy(this.noDashboard, this)).run({dontCheckCache: dontCache, dontProcessError: true});
	},
